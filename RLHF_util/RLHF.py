# RLHF components, adapted from "imitation.algorithms.preference_comparisons"
...
from sample_util.cus_rollout import cus_generate_trajectories
from wrapper_util import env_sample_wrapper
...
from stable_baselines3.common import type_aliases, utils, vec_env
...
from wrapper_util import cus_reward_wrapper
...
class AgentTrainer(TrajectoryGenerator):
    ...
    def __init__(
        ...
        self.buffering_wrapper = env_sample_wrapper.BufferingWrapper(venv)
        self.venv = self.reward_venv_wrapper = cus_reward_wrapper.RewardVecEnvWrapper(
...
class SoBiPreferenceModel(nn.Module):
    ...
    def __init__(
        ...
        threshold: float = 50,
        reduce_var: bool = True
    ) -> None:
        """Create Preference Prediction Model.

        Args:
            model: base model to compute reward.
            agent: agent to predict \pi, to reduce variance whem computing
                hyper-gradient term
    ...
    def forward(
        ...
        """
        probs = th.empty(len(fragment_pairs), dtype=th.float32, device='cuda')
        hypers = th.empty(len(fragment_pairs), dtype=th.float32, device='cuda')
        ...
        for i, fragment in enumerate(fragment_pairs):
            ...
            rews1, rew1_diff = self.rewards(trans1)
            rews2, rew2_diff = self.rewards(trans2)
            temp_crossentropy = self.probability(rews1, rews2)
            probs[i] = temp_crossentropy
            hypers[i] = (rew1_diff.sum()+rew2_diff.sum())
            if gt_reward_available:
        ...
        if self.agent.global_step <= 1e5:
            coef = 0
        else:
            coef = 1/self.agent.alpha
        return probs, (gt_probs if gt_reward_available else None), coef*hypers
    ...
    def rewards(self, transitions: Transitions) -> th.Tensor:
        ...
        state, action, next_state, done = preprocessed
        rews = self.model(*preprocessed)

        if self.reduce_var:
            with th.no_grad():
                sample_action, _, action_probs = actor.get_action(state)
            reward_distribution = self.model(state, action, next_state, done, True) # the three arguments action, next_state, done are useless
            sample_rews = (reward_distribution*action_probs).sum(dim=1)
        else:
            with th.no_grad():
                sample_action, _, _ = actor.get_action(state)
                sample_action = th.nn.functional.one_hot(sample_action, num_classes=9)
            sample_rews = self.model(state, sample_action, next_state, done) # the last two arguments are useless
            assert sample_rews.shape == (len(state),)
        assert rews.shape == (len(state),)
        return rews, rews-sample_rews
...
class RandomFragmenter(Fragmenter):
    ...
    def __init__(
        ...
        custom_logger: Optional[imit_logger.HierarchicalLogger] = None,
        terminate_prob: float = 0.00,  # Probability of selecting a fragment from the end of a trajectory
    ) -> None:
        """Initialize the fragmenter.
    ...
    def __call__(
        ...
        for _ in range(2 * num_pairs):
            ...
            n = len(traj)
            # Decide if the fragment should be taken from the end
            if self.rng.random() < self.terminate_prob and n >= fragment_length:
                start = n - fragment_length
            else:
...
class SyntheticGatherer(PreferenceGatherer):
    ...
    def __init__(
        ...
        self.rng = rng
        self.threshold = threshold
        self.consider_terminate = consider_terminate
    ...
    def __call__(self, fragment_pairs: Sequence[TrajectoryWithRewPair]) -> np.ndarray:
        ...
        if self.consider_terminate:
            neg_return1 = np.zeros(len(fragment_pairs))
            neg_return2 = np.zeros(len(fragment_pairs))
            for ind, pair in enumerate(fragment_pairs):
                if pair[0].terminal:
                    neg_return1[ind] = 100
                if pair[1].terminal:
                    neg_return2[ind] = 100
            returns1 -= neg_return1
            returns2 -= neg_return2
...
class RewardLoss(nn.Module, abc.ABC):
    ...
    def forward(
        ...
        fragment_pairs: Sequence[TrajectoryPair],
        preferences: np.ndarray,
        preference_model: SoBiPreferenceModel,
    ) -> LossAndMetrics:
        """Computes the loss.
...
class SoBiRewardLoss(RewardLoss):
    """Compute the cross entropy reward loss plus a hyper-gradient term."""
    ...
    def forward(
        ...
        Returns:
            The cross-entropy loss between the probability predicted by the
                ...
                available.
        """
        probs, gt_probs, hypers = preference_model(fragment_pairs)
        ...
        metrics = {}
        metrics["accuracy"] = (predictions.cpu() == ground_truth).float().mean()
        ...
        cross_entropy_flat = th.nn.functional.binary_cross_entropy(probs, preferences_th.to('cuda'),reduction='none')
        entropy_loss = cross_entropy_flat.mean()
        hyper_loss = (cross_entropy_flat.detach() * hypers).mean()
        return LossAndMetrics(
            loss=entropy_loss+hyper_loss,
            metrics=metrics,
        )
...
class SoBiRewardTrainer(RewardTrainer):
    """Train a SoBiRL reward model."""
    ...
    def _train(
        ...
        # lr_anneal = self._preference_model.agent.args.lr_anneal
        # if  lr_anneal > 0:
        #     frac = 1.0 - (self._preference_model.agent.global_step - 1.0) / lr_anneal
        #     reward_lrnow = frac * self.lr
        #     self.optim.param_groups[0]["lr"] = reward_lrnow
